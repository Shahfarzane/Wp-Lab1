	...
	class Bricks_WS_Form_Form extends \Bricks\Element {
		...
		public function render() {
			...
			if($form_id > 0) {
				...
				// Show shortcode
				$shortcode = sprintf('[ws_form id="%u"%s%s]', $form_id, ($form_element_id != '') ? sprintf(' element_id="%s"', esc_attr($form_element_id)) : '', ((bricks_is_builder_preview() || bricks_is_ajax_call()) ? ' visual_builder="true"' : ''));
